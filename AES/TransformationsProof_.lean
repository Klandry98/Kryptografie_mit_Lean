...
theorem gfMul_comm : ∀ a b : Byte, gfMul a b = gfMul b a := by decide
...
theorem gfMul_distrib_const : ∀ x y : Byte,
    ...
    (gfMul 0x0e (x ^^^ y) = gfMul 0x0e x ^^^ gfMul 0x0e y) := by decide +kernel   -- 6 * 256^2
...
--
-- Reiths Lektion (mem-/strukturbasiert): der Kern ist xor_xor_cancel je Byte;
-- die ofFn-Rekonstruktion traegt das auf den State.
...
-- ===========================================================================
--
-- Domaene 256^4 -> nicht per Enumeration. Bausteine vorhanden (Stufe A':
-- gfMul_distrib_const, invMatrix_mul_matrix_eq_id, xor_assoc/comm). Die
-- Zusammensetzung (Koeffizient jeder Eingabe = (M^{-1}M)[i][k] = delta) ist
-- eine laengere Gleichungskette von Hand und im Rahmen dieser Arbeit offen.
